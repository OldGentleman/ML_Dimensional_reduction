...
from read_image_file import read_images


target_food, images_food = read_images(
    'data/raw_data/food.zip', 'images/', (512, 512), 10)
...
for r_name, r_method in zip(reduction_names, reductions):
    ...
    if r_name == 'LDA':
        reduced_train = r_method.fit(train_food, train_target).transform(train_food)
    else:
